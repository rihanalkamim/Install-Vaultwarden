...
argon_token() {
  ...
  token=$(echo -n "$password" | argon2 somesalt -t 2 -m 16 -p 1 | grep Encoded | awk '{print $2}')
  echo "$token"
}
...
openssl ca -batch -config openssl.cnf -in $vaultdomain.csr -out $vaultdomain.pem -passin pass:"$passw"
#Create fullchain
